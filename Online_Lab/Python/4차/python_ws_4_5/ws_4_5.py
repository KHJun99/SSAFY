...
def is_validation(user):
    ...
    if user.get('company') in black_list:
        return 'blocked'  # 즉시 종료

    # 3. mail 형식 검사 (@ 포함 여부)
    if '@' not in str(user.get('mail', '')):
        errors.append('mail')

    # 4. name 길이 검사 (2 ~ 30)
    name_len = len(str(user.get('name', '')))
    if not (2 <= name_len <= 30):
        errors.append('name')

    # 5. website 개수 검사 (1개 이상)
    if not user.get('website') or len(user['website']) < 1:
...
# 실행 예시
result_users = create_user(user_data)
